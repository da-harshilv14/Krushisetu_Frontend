...
function Personal_info() {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                // 🔹 Basic user info
                setFullNameValue(data.full_name || "");
                setEmailValue(data.email_address || "");
                setMobileValue(data.mobile_number || "");
                setAadhaarValue(data.aadhaar_number || "");

                // 🔹 Bank info
                setIfscValue(data.ifsc_code || "");
                setAccountNumberValue(data.bank_account_number || "");

                // 🔹 Location info
                setStateValue(data.state || "");
                setDistrictValue(data.district || "");
                setTalukaValue(data.taluka || "");
                setVillageValue(data.village || "");

                // 🔹 Land details
                setUnitValue(data.unit || "");
                setSoilTypeValue(data.soil_type || "");

                // 🔹 File previews (land proof, PAN, Aadhaar, photo)
                if (data.land_proof) {
                    setInputFileInfo({
                        name: data.land_proof.split("/").pop(),
                        preview: data.land_proof,
                    });
                } else setInputFileInfo(null);
                ...
                } else setAadhaarFileInfo(null);

                if (data.profile_photo) {
                    setPhotoFileInfo({
                        name: data.profile_photo.split("/").pop(),
                        preview: data.profile_photo,
                    });
                } else setPhotoFileInfo(null);
    ...
    const handleSubmit = async () => {
        ...
        // Append text fields
        data.append("full_name", fullNameValue);
        data.append("email", emailValue);
        data.append("phone", mobileValue);
        data.append("aadhaar_number", aadhaarValue);
        data.append("state", stateValue);
        data.append("district", districtValue);
        data.append("taluka", talukaValue);
        data.append("village", villageValue);
        data.append("unit", unitValue);
        data.append("soil_type", soilTypeValue);
        data.append("bank_account", accountNumberValue);
        data.append("ifsc", ifscValue);

        // Append files
        if (photoFileInfo) data.append("profile_image", photoFileInfo);
        if (panFileInfo) data.append("pan_card", panFileInfo);
        if (aadhaarFileInfo) data.append("aadhaar_card", aadhaarFileInfo);
        if (inputFileInfo) data.append("land_proof", inputFileInfo);
